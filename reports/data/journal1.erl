...
%% @copyright 2017
%% @version 1.0.0
%% @doc Модуль <b>journal1.erl</b> реализует формирование данных
%% для документа печного журнала <Параметры хода печи>.
%% @end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-include("reports.hrl").
...
data({struct,Props}) ->
	...
	FieldsAndActions=[
		{"CHARGE",max},
		{"CHARGE_LEVEL_L_REF",max},
		{"CHARGE_LEVEL_L",max},
		{"CHARGE_LEVEL_R_REF",max},
		{"CHARGE_LEVEL_R",max},
		{"FHD",avg},
		{"TGD",avg},
		{"MHD",avg},
	...
	{Fields,Actions}=lists:unzip(FieldsAndActions),
	...
	{ok,
		[
			{"name","Печной журнал. Параметры хода печи."},
				...
				lists:map(
					fun(
						...
						[
							...
							FPG_HD,
							PPG_HD,
							iif(FHD==0.0,(catch FPG_HD/(FHD*60)*100.0),0.0)
						]
					end,
					Data
				),
