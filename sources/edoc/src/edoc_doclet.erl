...
-export([run/2,get_module_title/1]).
...
title(App, Options) ->
    proplists:get_value(title, Options,
			if App == ?NO_APP ->
				to_unicode(<<"Обзор">>);
			   true ->
				to_unicode(list_to_binary([<<"Приложение: ">>,io_lib:fwrite("~s",[App])]))
			end).
...
packages_frame(Dir, Ps, Title, CSS) ->
    Body = [?NL,
	    {h2, [{class, "indextitle"}], [to_unicode(<<"Пакеты">>)]},
	    ?NL,
	    {table, [{width, "100%"}, {border, 0},
		     {summary, to_unicode(<<"список пакетов">>)}],
	     lists:concat(
	       [[?NL,
...
get_module_title(M) ->
    case catch apply(M,get_module_name,[]) of
        {'EXIT',_ }->
            atom_to_list(M);
        Name ->
...
modules_frame(Dir, Ms, Title, CSS) ->
	{Ms1,Ms2}=lists:partition(
        fun(Elem) ->
            lists:prefix("custom_modules.",atom_to_list(Elem))
        end,
        Ms
    ),
    MsSorted1=lists:sort(
        fun(M1,M2) ->
            get_module_title(M1)=<get_module_title(M2)
        end,
        Ms1
    ),
    MsSorted2=lists:sort(
        fun(M1,M2) ->
            get_module_title(M1)=<get_module_title(M2)
        end,
        Ms2
    ...
    Body = [?NL,
	    {h2, [{class, "indextitle"}], [to_unicode(<<"Модули">>)]},
	    ?NL,
	    {table, [{width, "100%"}, {border, 0},
		     {summary, to_unicode(<<"список модулей">>)}],
...
app_index_file(Paths, Dir, Env, Options) ->
    Title = proplists:get_value(title, Options,to_unicode(<<"Обзор">>)),
