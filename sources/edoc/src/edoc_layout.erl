...
-define(NL, "\n").
-define(DESCRIPTION_TITLE, to_unicode(<<"Описание">>)).
-define(DESCRIPTION_LABEL, "description").
-define(DATA_TYPES_TITLE, to_unicode(<<"Типы данных">>)).
-define(DATA_TYPES_LABEL, "types").
-define(FUNCTION_INDEX_TITLE, to_unicode(<<"Список функций">>)).
-define(FUNCTION_INDEX_LABEL, "index").
-define(FUNCTIONS_TITLE, to_unicode(<<"Описание функций">>)).
...
layout_module(#xmlElement{name = module, content = Es}=E, Opts) ->
    ...
    Title = case get_elem(args, Es) of
		[] -> [to_unicode(<<"Модуль ">>), edoc_doclet:get_module_title(list_to_atom(Name))];
		_ -> [to_unicode(<<"Абстрактный модуль ">>), edoc_doclet:get_module_title(list_to_atom(Name)), " [", {Args}, "]"]
	    end,
    ...
    Body = (navigation("top")
	    ...
	    ++ [?NL]
	    ++ copyright2(Es)
	    ++ deprecated(Es, to_unicode(<<"модуль">>))
	    ++ [?NL]
	    ++ version(Es)
	    ++ since(Es)
	    ++ behaviours(Es, Name)
...
timestamp() ->
    [?NL, {p, [{i, [to_unicode(<<"Сгенерировано EDoc, ">>)++io_lib:fwrite("~s, ~s.",
...
function(Name, E=#xmlElement{content = Es}, Opts) ->
    ([?NL,
     ...
     ++ throws(Es, Opts)
     ++ equiv_p(Es)
     ++ deprecated(Es, to_unicode(<<"функция">>))
     ++ fulldesc(Es)
     ++ since(Es)
     ++ sees(Es)
     ++ todos(Es)).
...
throws(Es, Opts) ->
	...
	Es1 ->
	    ...
	    [{p, ([to_unicode(<<"исключение ">>), {tt, t_utype(get_elem(type, Es1))}]
...
typedef(Es, Opts) ->
    ...
    (case get_elem(type, Es) of
 	 [] -> [{b, [to_unicode(<<"абстрактный тип данных">>)]}, ": ", {tt, Name}];
...
sees(Es) ->
	...
	[] -> [];
	Es1 ->
	    [{p, [{b, [to_unicode(<<"Смотри также:">>)]}, " "] ++ seq(fun see/1, Es1, ["."])},
	     ?NL]
    end.
...
equiv(Es, P) ->
	...
	Es1 ->
		...
		[Expr] ->
		    ...
		    Txt = [to_unicode(<<"Эквивалентно ">>)] ++ Expr2 ++ ["."],
		    (case P of
			 true -> [{p, Txt}];
			 false -> Txt
		     end
...
copyright(Es) ->
	...
	[] -> [];
	Es1 ->
	    [{p, [to_unicode(<<"Все права сохранены (с) ">>) | Es1]}, ?NL]
    end.

copyright2(Es) ->
    case get_content2(copyright, Es) of
	[] -> [];
	Es1 ->
	    [{p, [to_unicode(<<"Все права сохранены (с) ">>) | Es1]}, ?NL]
    end.
...
since(Es) ->
	...
	[] -> [];
	Es1 ->
	    [{p, [{b, [to_unicode(<<"Представлено с:">>)]}, " " | Es1]}, ?NL]
    end.
...
deprecated(Es, S) ->
	...
	[] -> [];
	Es2 ->
	    [{p, [{b, [to_unicode(<<"Это ">>) ++ S ++ to_unicode(<<" устарело:">>)]}, " " | Es2]},
	     ?NL]
    end.
...
behaviours(Es, Name) ->
    (case get_elem(behaviour, Es) of
	 [] -> [];
	 Es1 ->
	     [{p, ([{b, [to_unicode(<<"Шаблон поведения:">>)]}, " "]
	 ...
	 [] -> [];
	 Es1 ->
	     [{p, ([{b, [to_unicode(<<"Этот модуль определяет ">>), {tt, [Name]},
			 to_unicode(<<" шаблон поведения.">>)]},
		    br, to_unicode(<<" Требуемые функции callbacks: ">>)]
...
authors(Es) ->
	...
	Es1 ->
		% io:format("~w~n",[Es1]),
	    [{p, [{b, [to_unicode(<<"Авторы:">>)]}, " "] ++ seq(fun author/1, Es1, ["."])},
	     ?NL]
    end.
...
authors2(Es) ->
	...
	Es1 ->
		% io:format("~w~n",[Es1]),
	    [{p, [{b, [to_unicode(<<"Авторы:">>)]}, " "] ++ seq(fun author2/1, Es1, ["."])},
	     ?NL]
    end.
...
author(E=#xmlElement{}) ->
%	io:format("~w~n",[io_lib:format("~ts",[list_to_binary(get_attrval(name, E))])]),
    Name = get_attrval(name, E),
...
author2(E=#xmlElement{}) ->
    Name = lists:flatten(io_lib:format("~ts",[list_to_binary(get_attrval(name, E))])),
    Mail = get_attrval(email, E),
    URI = get_attrval(website, E),
    (if Name == Mail ->
	     [{a, [{href, "mailto:" ++ Mail}],[{tt, [Mail]}]}];
...
get_elem2(Name, [#xmlElement{name = Name} = E | Es]) ->
	case Name of
		copyright ->
			[XmlText]=E#xmlElement.content,
			[E#xmlElement{content=[XmlText#xmlText{value=lists:flatten(io_lib:format("~ts",[list_to_binary(XmlText#xmlText.value)]))}]} | get_elem2(Name, Es)];
		_ ->
    		[E | get_elem2(Name, Es)]
    end;
get_elem2(Name, [_ | Es]) ->
...
get_content2(Name, Es) ->
    case get_elem2(Name, Es) of
	[#xmlElement{content = Es1}] ->
	    Es1;
	[] -> []
    end.
...
%	    ++ ShortDesc
	    ++ copyright(Es)
	    ++ deprecated(Es, to_unicode(<<"пакет">>))
	    ++ version(Es)
	    ++ since(Es)
	    ++ authors(Es)
	    ++ references(Es)
