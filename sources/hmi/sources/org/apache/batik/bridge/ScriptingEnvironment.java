...
public class ScriptingEnvironment extends BaseScriptingEnvironment {
    ...
    public void runEventHandler(String script, Event evt,
        ...
        final Interpreter interpreter = getInterpreter(lang);
        if (interpreter == null)
            return;
    ...
    protected class Window implements org.apache.batik.script.Window {
        ...
        public Object setInterval(final String script, long interval) {
            TimerTask tt = new TimerTask() {
                    ...
                    public void run() {
//                        System.out.println("starting: "+script+", eir.count="+String.valueOf(eir.count));
                        synchronized (eir) {
                            if (eir.count > 0) 
                                return;
                            eir.count++;
...
//                            System.out.println("invoking: "+script+", eir.count="+String.valueOf(eir.count));
            ...
            Thread t = new Thread() {
                    public void run() {
                        try {
                            ...
                            URL u=null;
                            try {
                                u=new URL(purl.toString());
                                url=u.getPath();
                            } catch (MalformedURLException ex) {
                                Logger.getLogger(ScriptingEnvironment.class.getName()).log(Level.SEVERE, null, ex);
                            }
                            final HttpGet httpget=new HttpGet(Main.getActiveBackend()+url);
                            System.out.println(httpget.toString());
                            HttpResponse response=httpClient.execute(httpget);
                            System.out.println(response.toString());
                            BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent(),"UTF-8"));
                            System.out.println(httpget.toString()+" new BufferedReader. ");
                            final StringBuffer sb = new StringBuffer();
                            int read;
                            char[] buf = new char[4096];
                            while ((read = rd.read(buf, 0, buf.length)) != -1) {
                                System.out.println(httpget.toString()+" reading buffer. ");
                                sb.append(buf, 0, read);
                            }
                            System.out.println(httpget.toString()+" closing BufferedReader. ");
                            rd.close();
                            System.out.println(httpget.toString()+" finished. "+String.valueOf(sb.length()));

                            updateRunnableQueue.invokeLater(new Runnable() {
                                    public void run() {
                                        try {
                                            System.out.println(httpget.toString()+" calling backend. ");
                                            h.getURLDone(true,
                                                         purl.getContentType(),
                                                         sb.toString());
                                        } catch (Exception e){
            ...
            if (userAgent != null) {
                ...
                try {
                    if (base == null) {
                        ...
                        url2 = new URL(new URL(base), url);
                    }
                } catch (MalformedURLException ex) {
                    Logger.getLogger(ScriptingEnvironment.class.getName()).log(Level.SEVERE, null, ex);
                }
                userAgent.showOpenURL(url2.toExternalForm(),newwindow);
            }
        }
        
        public void openURLWithParameters(String url,boolean newwindow,String initialScript) {
            if (userAgent != null) {
                String base =
                    ((SVGOMDocument)document).getDocumentURI();
                URL url2=null;
                ...
                userAgent.showOpenURLWithParameters(url2.toExternalForm(),newwindow,initialScript);
            }
        }
