...
public class AbstractJSVGComponent extends JGVTComponent {
    ...
    public static final String SCRIPT_OPENURL = "script.openURL";
    ...
    public AbstractJSVGComponent(SVGUserAgent ua, boolean eventsEnabled,
        ...
        svgUserAgent = ua;
        userAgent = new BridgeUserAgentWrapper(new BridgeUserAgent());
    ...
    public void loadSVGDocument(String url) {
        ...
        stopThenRun(new Runnable() {
                public void run() {
                    ...
                    loader.dispose();
                    loader=null;
                    nextDocumentLoader.setPriority(Thread.MAX_PRIORITY);
    ...
    protected void installSVGDocument(SVGDocument doc) {
        ...
        nextGVTTreeBuilder.setPriority(Thread.MAX_PRIORITY);
...
//        addComponentListener(jsvgComponentListener);
    ...
    public void addKeyAction(FunctionWrapper fw,long key, long mask) {
    ...
    protected class SVGListener
        ...
        public void documentLoadingFailed(SVGDocumentLoaderEvent e) {
            ...
            userAgent.displayError(((SVGDocumentLoader)e.getSource()).
                                   getException());
            JOptionPane.showMessageDialog(null, ((SVGDocumentLoader)e.getSource()).
                                   getException());
...
//            removeComponentListener(jsvgComponentListener);
        ...
        public void managerStarted(final UpdateManagerEvent e) {
            EventQueue.invokeLater(new Runnable() {
                    public void run() {
                        ...
                        if (updateManagerListeners!=null) {
    ...
    protected static class BridgeUserAgentWrapper implements UserAgent {
        ...
        public void showSetFullscreen(final boolean fullscreen) {
            if (EventQueue.isDispatchThread()) {
                userAgent.showSetFullscreen(fullscreen);
            } else {
        ...
        public void addKeyAction(final FunctionWrapper fw,final long key,final long mask) {
            if (EventQueue.isDispatchThread()) {
                userAgent.addKeyAction(fw,key,mask);
            } else {
                invokeAndWait(new Runnable() {
                        public void run() {
                            userAgent.addKeyAction(fw,key,mask);
        ...
        public void showPopupMenu(final JPopupMenu menu) {
            if (EventQueue.isDispatchThread()) {
                userAgent.showPopupMenu(menu);
            } else {
                invokeAndWait(new Runnable() {
                        public void run() {
                            userAgent.showPopupMenu(menu);
        ...
        public void showOpenURL(final String url,final boolean newwindow) {
            if (EventQueue.isDispatchThread()) {
                userAgent.showOpenURL(url,newwindow);
            } else {
                invokeAndWait(new Runnable() {
                        public void run() {
                            userAgent.showOpenURL(url,newwindow);
        ...
        public void showOpenURLWithParameters(final String url,final boolean newwindow,final String initialScript) {
            if (EventQueue.isDispatchThread()) {
                userAgent.showOpenURL(url,newwindow);
            } else {
                invokeAndWait(new Runnable() {
                        public void run() {
                            userAgent.showOpenURLWithParameters(url,newwindow,initialScript);
                        }
                    });
            }
        }
    ...
    protected class BridgeUserAgent implements UserAgent {
        ...
        public void showSetFullscreen(boolean fullscreen) {
            if (svgUserAgent != null) {
                svgUserAgent.showSetFullscreen(fullscreen);
                return;
            }
            AbstractJSVGComponent.this.showSetFullscreen(fullscreen);
        }
        
        public void addKeyAction(FunctionWrapper fw,long key, long mask) {
            if (svgUserAgent != null) {
                svgUserAgent.addKeyAction(fw,key,mask);
                return;
            }
            AbstractJSVGComponent.this.addKeyAction(fw,key,mask);
        }

        public void showPopupMenu(JPopupMenu menu) {
            if (svgUserAgent != null) {
                svgUserAgent.showPopupMenu(menu);
                return;
            }
            AbstractJSVGComponent.this.showPopupMenu(menu);
        }


        public void showCloseWindow() {
            if (svgUserAgent != null) {
                svgUserAgent.showCloseWindow();
                return;
            }
            AbstractJSVGComponent.this.showCloseWindow();
        }

        public void showOpenURL(String url,boolean newwindow) {
            if (svgUserAgent != null) {
                svgUserAgent.showOpenURL(url,newwindow);
                return;
            }
            AbstractJSVGComponent.this.showOpenURL(url,newwindow);
        }
        ...
        public void showOpenURLWithParameters(String url, boolean newwindow, String initialScript) {
            if (svgUserAgent != null) {
                svgUserAgent.showOpenURLWithParameters(url,newwindow,initialScript);
                return;
            }
            AbstractJSVGComponent.this.showOpenURLWithParameters(url,newwindow,initialScript);
        }
