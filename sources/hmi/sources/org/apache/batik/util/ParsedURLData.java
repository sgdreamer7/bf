...
//        System.out.println("ParsedURLData()");
    }
...
loop2:          while (i < len) {
    ...
    protected InputStream openStreamInternal(String userAgent,
        ...
        if (url.toExternalForm().startsWith("http://")==false) {
        ...
        if (mimeTypes != null) {
            ...
            params.setParameter(HTTP_ACCEPT_HEADER, acceptHeader);
        }
        if (encodingTypes != null) {
            ...
            params.setParameter(HTTP_ACCEPT_ENCODING_HEADER,
                                    encodingHeader);
        }
        ...
        try {
            ...
            return (stream = response.getEntity().getContent());
        } catch (Exception e) {
            throw new IOException
                ("Invalid uri: "+url.toExternalForm());
        }
