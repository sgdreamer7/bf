-module(rest_cowboy).
-record(st, {resource_module = undefined :: atom(), resource_id = undefined :: binary(), params = [] :: list() }).
...
rest_init(Req, _Opts) ->
    ...
    {Params,Req3}=case cowboy_req:path_info(Req2) of
        {undefined,Rq} ->
            {[],Rq};
        {ParamsList,Rq} ->
            {ParamsList,Rq}
    end,
    Req4 = wf:header(<<"Access-Control-Allow-Origin">>, <<"*">>, Req3),
    {ok, Req4, #st{resource_module = Module, resource_id = Id, params = Params}}.
...
to_html(Req, #st{resource_module = M, resource_id = Id, params = Params} = State) ->
    Body = case Id of
               undefined -> [M:to_html(Resource) || Resource <- M:get()];
               _ -> M:to_html(M:get(Id,Params)) end,
...
to_json(Req, #st{resource_module = M, resource_id = Id, params = Params} = State) ->
    Struct = case Id of
                 undefined -> {struct, [{M, [{struct, M:to_json(Resource)} || Resource <- M:get()]}]};
                 _         -> {struct, M:to_json(M:get(Id,Params))} end,
