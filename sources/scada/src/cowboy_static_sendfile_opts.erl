...
%%      {file, "other.html"}]}
%% '''
-module(cowboy_static_sendfile_opts).
...
rest_init(Req, Opts) ->
	ChunkSize=proplists:get_value(chunk_size,Opts,1024*100),
	...
	case lists:keyfind(file, 1, Opts) of
		false ->
			...
			case fullpath(Filepath) of
				...
				_ ->
					{ok, Req2, #state{chunk_size=ChunkSize,filepath=error, fileinfo=error,
...
rest_init(Req, Opts, Filepath) ->
	...
	{ok, Req, #state{chunk_size=ChunkSize,filepath=Filepath, fileinfo=Fileinfo,
...
file_contents(Req, #state{chunk_size=ChunkSize,filepath=Filepath,
		fileinfo={ok, #file_info{size=Filesize}}}=State) ->
	Writefile = fun(Socket, _Transport) ->
		%% Transport:sendfile/2 may return {error, closed}
		%% if the connection is closed while sending the file.
		case sendfile(Filepath, Socket,[{chunk_size,ChunkSize}]) of
			{ok, _} -> ok;
			{error, closed} -> ok
		end
	end,
