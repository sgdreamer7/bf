...
%% @copyright 2017
%% @version 1.1.0
%% @doc Модуль <b>{@module}</b> реализует драйвер для симулированных значений тегов.
%% @end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-module(scada_driver_simulator).
...
init(_) ->
	...
	?log_sys(io_lib:format("scada_driver_simulator starting...~n",[])),
	State=#state{
		callbacks=gb_trees:empty()
	},
	cron:add(scada_driver_simulator_updater,fun updater/1,100,0,undefined),
	?log_sys(io_lib:format("scada_driver_simulator started.~n",[])),
	{ok, State}.
...
handle_info(Other, State) ->
	?log_sys(io_lib:format("scada_driver_simulator Unknown message ~p~n",[Other])),
	{noreply,State}.
...
terminate(_Reason, _State) ->
	...
	cron:remove(scada_driver_simulator_updater),
	normal.
...
process_callbacks(none) ->
	ok;
process_callbacks({_Address,TagsCallbacks,Iterator}) ->
	TagsIterator=gb_trees:iterator(TagsCallbacks),
	process_tags_callbacks(gb_trees:next(TagsIterator)),
	process_callbacks(gb_trees:next(Iterator)).

process_tags_callbacks(none) ->
	ok;
process_tags_callbacks({TagName,Callback,TagsIterator}) ->
	{A1,A2,A3}=now(),
	random:seed(A1,A2,A3),
	Value=0.01*math:sin((A2 rem 5)/5*2*math:pi())+0.010*random:uniform()+0.50,
	case scada_tags:get_field(TagName,"A_ETAG") of
		"ANALOG" ->
			LowBound=scada_tags:get_field(TagName,"F_LOLO"),
			HighBound=scada_tags:get_field(TagName,"F_HIHI"),
			Callback(TagName,Value*(HighBound-LowBound)+LowBound);
		"DIGITAL" ->
			Callback(TagName,((A2 div 2) rem 2)==0);
		"STRING" ->
			Callback(TagName,"");
		"ARRAY" ->
			Callback(TagName,[]);
