%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% @doc Модуль <b>{@module}</b> реализует функциональность соединения с сервером баз данных MySQL.
%% @end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-module(mysql_conn).
...
convert_type(Val, ColType) ->
	...
	T when T == 'TIMESTAMP';
	       T == 'DATETIME' ->
	    {ok, [Year, Month, Day, Hour, Minute, Second, Millisecond], _Leftovers} =
		io_lib:fread("~d-~d-~d ~d:~d:~d.~d", binary_to_list(Val)),
	    case Millisecond of
	    	0 ->
